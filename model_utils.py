...
class BasicTransBlock(nn.Module):
    ...
    def __init__(self, in_ch, heads, dim_head, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp',
        ...
        if proj_att:
            ...
            self.attn = StandardAttention(in_ch, heads=heads, dim_head=in_ch // heads, attn_drop=attn_drop,
                                        proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                        rel_pos=rel_pos)
...
class BasicTransDecoderBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, heads, dim_head, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp',
        ...
        if proj_att:
            ...
            self.attn = StandardAttentionDecoder(in_ch, out_ch, heads=heads, dim_head=out_ch // heads, attn_drop=attn_drop,
                                               proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                               rel_pos=rel_pos)
...
class StandardAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 调整q、k、v维度，使其适合矩阵乘法（这里假设H和W为特征图的高和宽）
        q = q.view(B, self.heads, H * W, self.dim_head)
        k = k.view(B, self.heads, H * W, self.dim_head)
        v = v.view(B, self.heads, H * W, self.dim_head)

        # 计算注意力得分矩阵（通过矩阵乘法实现点积操作）
        q_k_attn = torch.matmul(q, k.transpose(-1, -2))
        q_k_attn *= self.scale
        q_k_attn = F.softmax(q_k_attn, dim=-1)
        q_k_attn = self.attn_drop(q_k_attn)

        # 计算输出
        out = torch.matmul(q_k_attn, v)
...
class StandardAttentionDecoder(nn.Module):
    def __init__(self, in_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16,
                 projection='interp', rel_pos=True):
        super().__init__()
    ...
    def forward(self, q, x):
        ...
        # 通过双线性插值将k和v的空间维度调整为和q一致（这里统一到16, 16）
        k = F.interpolate(k, size=(HH, WH), mode='bilinear', align_corners=True)
        v = F.interpolate(v, size=(HH, WH), mode='bilinear', align_corners=True)

        q = self.to_q(q)  # BH, inner_dim, HH, WH

        # 调整q、k、v维度，使其适合矩阵乘法（这里假设H和W为特征图的高和宽）
        q = q.view(BH, self.heads, HH * WH, self.dim_head)
        k = k.view(BH, self.heads, HH * WH, self.dim_head)
        v = v.view(BH, self.heads, HH * WH, self.dim_head)
        ...
        # 计算输出
        out = torch.matmul(q_k_attn, v)
        out = out.view(BH, self.inner_dim, HH, WH)
        out = self.to_out(out)
        out = self.proj_drop(out)
...
class down_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, maxpool=True, heads=4, dim_head=64, attn_drop=0.,
                 proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, proj_att=True):
        ...
        if maxpool:
            ...
            # block_list.append(nn.Conv2d(in_ch, out_ch, stride=1,kernel_size=1))
        else:
            block_list.append(block(in_ch, out_ch, stride=1))
            # block_list.append(nn.Conv2d(in_ch, out_ch, stride=1, kernel_size=1))
...
class up_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0.,
                 reduce_size=16, projection='interp', rel_pos=True, proj_att=True):
        super().__init__()
        ...
        # block_list.append(nn.Conv2d(2 * out_ch, out_ch, stride=1,kernel_size=1))
        self.blocks = nn.Sequential(*block_list)
...
class down_block(nn.Module):
    def __init__(self, in_ch, out_ch, scale, num_block, bottleneck=False, pool=True):
        ...
        for i in range(num_block-1):
            ...
            # block_list.append(nn.Conv2d(out_ch, out_ch, stride=1, kernel_size=1))
...
if __name__ == '__main__':
    input1 = torch.rand(2, 512, 8, 8)
    input2 = torch.rand(2, 256, 16, 16)
    block = up_block_trans(16 * 32, 8 * 32, num_block=1, bottleneck=None,
                           heads=4, dim_head=8 * 32 // 4,
                           attn_drop=0.1, proj_drop=0.1, reduce_size=8,
                           projection='interp', rel_pos=True)

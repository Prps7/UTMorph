...
from utils import warp, save_flow, reverse_warp
import argparse
...
def parse_args():
    ...
    parser.add_argument('--batchsize', type=int, default=24, help='Batch size for training and validation')
    parser.add_argument('--checkpoints', type=str, default="./output/beta10.pth", help='Checkpoints for training')
...
if __name__ == "__main__":
    ...
    for batch_data in valid_loader:
        ...
        pred_image = reverse_warp(fixed_image,ddf)
        pre_label = reverse_warp(fixed_label,ddf)

        dice, msd, hd = calculate_metrics(pre_label, moving_label)
        metric.extend(dice)
        HD.append(hd)
        MSD.append(msd)
